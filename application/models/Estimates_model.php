...
class Estimates_model extends App_Model
{
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if ($client == true) {
                ...
                if ($action == 4) {
                    ...
                    if ($invoiced) {
                        $this->invoices_model->send_invoice_to_client($invoiceid, '', true, '', true);
                    }
     ...
     * @param bool   $attachpdf     attach estimate pdf or not
...
public function send_estimate_to_client($id, $template_name = '', $attachpdf = true, $cc = '', $manually = false)
{
    ...
    if (is_array($send_to) && count($send_to) > 0) {
        ...
        // -------------------------------------------------------------------
        // EXTRA ATTACHMENTS - HANDLE ALL UPLOADED FILES
        // -------------------------------------------------------------------

        $uploaded_extra_files = [];

        if (!empty($_FILES['extra_attachments']['name'])) {

            $countFiles = count($_FILES['extra_attachments']['name']);

            for ($f = 0; $f < $countFiles; $f++) {

                if ($_FILES['extra_attachments']['name'][$f] == '') {
                    continue;
                }

                $_FILES['file']['name']     = $_FILES['extra_attachments']['name'][$f];
                $_FILES['file']['type']     = $_FILES['extra_attachments']['type'][$f];
                $_FILES['file']['tmp_name'] = $_FILES['extra_attachments']['tmp_name'][$f];
                $_FILES['file']['error']    = $_FILES['extra_attachments']['error'][$f];
                $_FILES['file']['size']     = $_FILES['extra_attachments']['size'][$f];

                $this->load->library('upload');

                $config['upload_path']   = FCPATH . 'uploads/estimate_email_attachments/';
                ...
                if ($this->upload->do_upload('file')) {
                    ...
                    $uploaded_extra_files[] = $config['upload_path'] . $data['file_name'];
                }
            }
        }
        ...
        foreach ($send_to as $contact_id) {
            if ($contact_id != '') {
                ...
                // -------------------------------------------------------------------
                // ADD EXTRA ATTACHMENTS TO EMAIL TEMPLATE
                // -------------------------------------------------------------------
                if (!empty($uploaded_extra_files)) {
                    foreach ($uploaded_extra_files as $filepath) {
                        $template->add_attachment([
