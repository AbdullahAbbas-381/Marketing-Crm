...
<div class="modal fade email-template"
    ...
    <div class="modal-dialog" role="document">
        ...
        <!-- IMPORTANT: multipart form for file uploads -->
        <?= form_open_multipart('admin/estimates/send_to_email/' . $estimate->id); ?>

        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close close-send-template-modal"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel">
                    <span class="edit-title"><?= _l('estimate_send_to_client_modal_heading'); ?></span>
                </h4>
            </div>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="checkbox checkbox-primary">
                            ...
                            <label for="attach_pdf"><?= _l('estimate_send_to_client_attach_pdf'); ?></label>
                        </div>

                        <h5 class="bold"><?= _l('estimate_send_to_client_preview_template'); ?></h5>
                        <hr />
                ...
                <?php if (count($estimate->attachments) > 0) { ?>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <h5 class="bold no-margin"><?= _l('include_attachments_to_email'); ?></h5>
                            <hr />
                            <?php foreach ($estimate->attachments as $attachment) { ?>
                                <div class="checkbox checkbox-primary">
                                    ...
                                    <label>
                                        <a href="<?= site_url('download/file/sales_attachment/' . $attachment['attachment_key']); ?>">
                ...
                <!-- ############################################################################################ -->
                <!-- ADDITIONAL DYNAMIC ATTACHMENTS (same system as tickets) -->
                <!-- ############################################################################################ -->

                <hr />
                <h5 class="bold">Additional Attachments</h5>
                ...
                <div class="input-group" style="margin-bottom:10px;">
                    <span class="input-group-btn">
                        <button class="btn btn-success add_more_estimate_attachments p8-half"
                ...
                <script>
                    $(function () {
                        ...
                        $('body').on('click', '.add_more_estimate_attachments', function () {
                            var max = $(this).data('max');
                            var total = $('.attachments .attachment').length;
                                ...
                                <div class="attachment">
                                    <div class="form-group">
                                        <input type="file"
                                               ...
                                               filesize="<?= file_upload_max_size(); ?>"
                                               class="form-control"
                                               name="extra_attachments[` + attachmentIndex + `]"
                                               accept="<?= get_ticket_form_accepted_mimes(); ?>">
                                    </div>
                                </div>`;
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default close-send-template-modal"><?= _l('close'); ?></button>
