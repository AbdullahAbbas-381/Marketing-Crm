...
class Invoices_model extends App_Model {
    ...
    public function add($data, $expense = false, $playground = false) {
        ...
        if ($insert_id) {
            ...
            $this->misc_model->handle_tags_save($tags, $insert_id, 'invoice');
            ...
            foreach ($items as $key => $item) {
                if ($itemid = $this->invoice_items_model->add_new_sales_item_post($item, $insert_id, 'invoice', $playground)) {
                    ...
                    $this->invoice_items_model->_maybe_insert_post_item_tax($itemid, $item, $insert_id, 'invoice', $playground);
                }
            }
    ...
    public function copy($id, $playground = false) {
        ...
        foreach ($_invoice->items as $item) {
            ...
            foreach ($custom_fields_items as $cf) {
                $new_invoice_data['newitems'][$key]['custom_fields']['items'][$cf['id']] = $this->custom_fields_model->get_custom_field_value($item['id'], $cf['id'], 'items', false, $playground);
        ...
        if ($id) {
            ...
            $this->load->model('custom_fields_model');
            $custom_fields = $this->custom_fields_model->get_custom_fields('invoice', [], false, $playground);
            foreach ($custom_fields as $field) {
                $value = $this->custom_fields_model->get_custom_field_value($_invoice->id, $field['id'], 'invoice', false, $playground);
                if ($value == '') {
                    continue;
                }
            ...
            $this->load->model('misc_model');
            $this->misc_model->handle_tags_save($tags, $id, 'invoice');
    ...
    public function update($data, $id, $playground = false) {
        ...
        $this->load->model('custom_fields_model');
        if ($this->custom_fields_model->handle_custom_fields_post($id, $custom_fields = $data['custom_fields'] ?? [], false, $playground)) {
            $updated = true;
        }
        $this->load->model('misc_model');
        if (array_key_exists('tags', $data)) {
            if ($this->misc_model->handle_tags_save($tags = $data['tags'], $id, 'invoice')) {
                $updated = true;
            }
        }
    ...
    protected function add_new_items($items, $billed_tasks, $billed_expenses, $id, $playground = false) {
        ...
        $this->load->model('invoice_items_model');
        foreach ($items as $key => $item) {
            if ($new_item_added = $this->invoice_items_model->add_new_sales_item_post($item, $id, 'invoice', $playground)) {
                if (isset($billed_tasks[$key])) {
                    foreach ($billed_tasks[$key] as $_task_id) {
                ...
                $this->invoice_items_model->_maybe_insert_post_item_tax($new_item_added, $item, $id, 'invoice', $playground);
    ...
    protected function save_items($items, $id, $playground = false) {
        ...
        foreach ($items as $item) {
            ...
            if (!isset($item['taxname']) || (isset($item['taxname']) && count($item['taxname']) == 0)) {
                ...
                $this->load->model('invoice_items_model');
                if ($this->invoice_items_model->_maybe_insert_post_item_tax($item['itemid'], $item, $id, 'invoice', $playground)) {
                    $updated = true;
                }
            }
        }
    ...
    public function delete_attachment($id, $playground = false) {
        ...
        if ($attachment) {
            if (empty($attachment->external)) {
                unlink($this->misc_model->get_upload_path_by_type('invoice', $playground) . $attachment->rel_id . '/' . $attachment->file_name);
            }
            ...
            if (is_dir($this->misc_model->get_upload_path_by_type('invoice', $playground) . $attachment->rel_id)) {
                // Check if no attachments left, so we can delete the folder also
                $other_attachments = list_files($this->misc_model->get_upload_path_by_type('invoice', $playground) . $attachment->rel_id);
                if (count($other_attachments) == 0) {
                    // okey only index.html so we can delete the folder also
                    delete_dir($this->misc_model->get_upload_path_by_type('invoice', $playground) . $attachment->rel_id);
                }
            }
        }
        return $deleted;
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if ($simpleDelete == false) {
                ...
                if (total_rows(db_prefix() . ($playground ? 'playground_' : '') . 'estimates', ['invoiceid' => $id, ]) > 0) {
                    ...
                    $this->estimates_model->log_estimate_activity($estimate->id, 'not_estimate_invoice_deleted', $playground);
                }
            }
            $this->db->select('id');
            ...
            foreach ($linked_credit_notes as $credit_note) {
                $this->credit_notes_model->update_credit_note_status($credit_note['id'], $playground);
            }
            $this->db->where('rel_id', $id);
            $this->db->where('rel_type', 'invoice');
            $this->db->delete(db_prefix() . ($playground ? 'playground_' : '') . 'notes');
            $this->load->model('misc_model');
            $this->misc_model->delete_tracked_emails($id, 'invoice', $playground);
            $this->db->where('rel_type', 'invoice');
            $this->db->where('rel_id', $id);
    ...
    public function send_invoice_overdue_notice($id, $playground = false) {
        ...
        if ($email_sent || $sms_sent) {
            if ($email_sent) {
                ...
                $this->log_invoice_activity($id, 'user_sent_overdue_reminder', false, serialize(['<custom_data>' . implode(', ', $emails_sent) . '</custom_data>', defined('CRON') ? ' ' : $this->staff_model->get_staff_full_name('', $playground), ]), $playground);
            }
            if ($sms_sent) {
    ...
    public function send_invoice_due_notice($id, $playground = false) {
        ...
        if ($email_sent || $sms_sent) {
            if ($email_sent) {
                $this->load->model('staff_model');
                $this->log_invoice_activity($id, 'activity_due_reminder_is_sent', false, serialize(['<custom_data>' . implode(', ', $emails_sent) . '</custom_data>', defined('CRON') ? ' ' : $this->staff_model->get_staff_full_name('', $playground), ]), $playground);
            }
            if ($sms_sent) {
    ...
    public function log_invoice_activity($id, $description = '', $client = false, $additional_data = '', $playground = false) {
        if (DEFINED('CRON')) {
            ...
            $this->load->model('staff_model');
            $full_name = $this->staff_model->get_staff_full_name(get_staff_user_id(), $playground);
        }
