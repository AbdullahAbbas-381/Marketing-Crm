...
function send_sms_reminder($to, $message)
{
	...
	if ($gateway !== false) {
		...
		$success = $this->{$className}->send($to, $message);
		if($success){
			return json_encode([
				'success' => true,
				'sent' => $sms_ids,
			...
			log_activity('Reminder module: SMS has sent to: '.$to);
		}
		log_activity('Reminder module: SMS failed to: '.$to.' send!');
	}
	log_activity('Reminder module: Unable to send SMS to: '.$to.' gateway is not active!');
}
...
function validateStatusOfRelation($relType, $relID)
{
	$CI  = &get_instance();
	$today = new DateTime();
	$today->setTime(0, 0);
	...
	if ($relType == 'proposal') {
		...
		if ((isset($result) && !empty($result)) && $result->status == 4 || $result->status == 1 || $result->status == 5 || $result->status == 3) {
			$return = true;
		}
	}
	if ($relType == 'estimate') {
		$CI->db->where('id', $relID);
		$result = $CI->db->get(db_prefix() . 'estimates')->row();
		if ((isset($result) && !empty($result)) && $result->status == 2 || $result->status == 4) {
			$return = true;
		}
	}
	if ($relType == 'invoice') {
		...
		if ((isset($result) && !empty($result)) && $result->status == 1 || $result->status == 3 || $result->status == 4) {
			$return = true;
		}
	}
	if ($relType == 'contract') {
	...
	if ($relType == 'ticket') {
		$CI->db->where('id', $relID);
		$result = $CI->db->get(db_prefix() . 'tickets')->row();
		if ((isset($result) && !empty($result)) && $result->status != 5) {
			$return = true;
		}
	}
	if ($relType == 'custom_reminder' || $relType == 'service') {
		$return = true;
	}
	return $return;
}
function rm_get_lead($id = null)
{
	$CI  = &get_instance();
	if (empty($id) && isset($GLOBALS['rm_lead'])) {
		return $GLOBALS['rm_lead'];
	}
	...
	$lead = get_instance()->leads_model->get($id);
