...
class Lead_manager extends AdminController
{
    ...
    public function generateClientTokenOld()
    {
    ...
    {
        $staffid = get_staff_user_id();
        $identity = 'support_agent_' . $staffid;
        $token = new AccessToken(get_option('call_twilio_account_sid'),get_option('call_twilio_api_key'),get_option('call_twilio_api_secret'),3600,$identity);
        $voiceGrant = new VoiceGrant();
        $voiceGrant->setOutgoingApplicationSid(get_option('call_twiml_app_sid'));
    ...
    public function serch_contacts_by_name()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->input->post('type') == 'lead') {
                ...
                $is_whatsapp = false;
                if ($this->input->post('is_whatsapp')) {
                    $is_whatsapp = true;
                }
                $orderBy['cond'] = 'chats.sms_date';
                $orderBy['order'] = 'DESC';
                $leads = $this->lead_manager_model->get('', $where, $orderBy, array('limit' => 100, 'start' => 0), $is_whatsapp);
            ...
            try {
                ...
                ->create(
                    ...
                    [
                      ...
                      'contentVariables' => $template->content_variables ? $template->content_variables : [],
                    ]
                  );
    ...
    {
        echo handle_whatsapp_file_uploads($sender_id, $sender_type);
        die;
    }
    ...
    public function ai_chat_bot()
    {
        ...
        $data['clients'] = $this->lead_manager_model->get_client('', ['addedfrom' => get_staff_user_id()], array('limit' => 100, 'start' => 0));
        $data['staff'] = get_staff();
        $data['title'] = _l('lead_manager_ai_chat_bot');
        $this->app_css->add('lm-ai-chat-css', 'modules/lead_manager/assets/css/ai_conversation.css');
        $this->load->view('admin/leads/ai_conversations', $data);
    }


    public function asst_config()
    {
        $data = $this->input->post();
        if (empty($data)) {
            set_alert('danger', 'Data Not received.');
            return;
        }
        $result = $this->lead_manager_model->add_or_update_ai_configuration($data);
        if ($result['success']) {
            set_alert('success', $result['message']);
        } else {
            set_alert('danger', $result['message']);
        }
        redirect(admin_url('settings?group=lead_manager_misc'));
    }

    public function ai_public_form($id = '')
    {
        if (!is_admin()) {
            access_denied('Web To Lead Access');
        }
        if (!get_option('lm_firebase_api_key') || !get_option('lm_firebase_auth_domain') || !get_option('lm_firebase_project_id') || !get_option('lm_firebase_storage_bucket') || !get_option('lm_firebase_messaging_sender_id') || !get_option('lm_firebase_app_id') || !get_option('lm_firebase_databse_url')) {
            set_alert('danger', 'Please setup Firebase!');
            redirect(admin_url('settings?group=lead_manager_misc'));
        }
        $formid = $this->lead_manager_model->get_chat_form_id();
        if (!$formid) {
            $data['form_key'] = $this->lead_manager_model->generate_form_key();
        }
        $id = $formid->id;
        if ($id != '') {
            $data['form'] = $this->lead_manager_model->get_chat_form(['id' => $id]);
            if (isset($data['form'])) {
                $data['title'] = _l('lm_ai_chat_bot');
                $data['formData'] = $data['form']->form_data;
            }
        }
        $this->load->view('lead_manager/forms/ai_chat_bot', $data);
    }
    ...
    public function save_form_data()
    {
        ...
        $this->db->update(db_prefix() . 'lm_ai_chat_form', [
            'form_data' => $data['formData'],
        ]);
        if ($this->db->affected_rows() > 0) {
            echo json_encode([
                'success' => true,
                'message' => _l('updated_successfully', _l('ai_chat_bot_form')),
            ]);
        } else {
            echo json_encode([
                'success' => false,
    ...
    public function send_test_email()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $template->message  = get_option('email_header') . 'This is test SMTP email. <br />If you received this message that means that your Lead manager mailbox SMTP settings is set correctly.' . get_option('email_footer');
            ...
            $this->email->from(config_item('smtp_fromname'), $template->fromname);
            ...
            if ($this->email->send(true)) {
                ...
                $response = [
                    'status' => 'danger',
                    'message' => '<h1>Your SMTP settings are not set correctly here is the debug log.</h1><br />' . $this->email->print_debugger() . (isset($GLOBALS['debug']) ? $GLOBALS['debug'] : '')
                ];
            }
            echo json_encode($response);
            die;
    ...
    public function test_email_imap_integration()
    {
        $response = [
            'status' => 'danger',
            'message' => 'Something went wrong!',
        ...
        if ($this->input->is_ajax_request()) {
            if (!is_admin()) {
                $response = [
                    'status' => 'danger',
                    'message' => 'Test Email IMAP Integration',
                ];
            }
            app_check_imap_open_function();
            $mail     = $this->lead_manager_model->get_mail_box_configuration(get_staff_user_id());
            ...
            try {
                ...
                try {
                    $folder = !empty($mail->imap_folder) ? $mail->imap_folder : 'INBOX';
                    $connection->getMailbox($folder);
                    $response = [
                        'status' => 'success',
                ...
                $response = [
                    'status' => 'danger',
                    'message' => _l('lead_email_connection_not_ok') . '<br /><br /><b>' . $error . '</b>',
                ];
            }
            echo json_encode($response);
            die;
