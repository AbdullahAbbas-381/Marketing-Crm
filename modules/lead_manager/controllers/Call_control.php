...
class Call_control extends CI_Controller
{
    ...
    public function handleCall()
    {
        ...
        log_activity(json_encode($_GET));
        $callerIdNumber = isset($_GET['From']) ? $_GET['From'] : get_option('call_twilio_phone_number');
        $leadid = (isset($_GET['leadId'])) ? $_GET['leadId'] : null;
        $is_lead = (isset($_GET['isLead'])) ? $_GET['isLead'] : null;
        $client_id = (isset($_GET['clientId'])) ? $_GET['clientId'] : null;
        $phoneNumberToDial = isset($_GET['To']) ? $_GET['To'] : null;
        $direction = $_GET['Direction'];
        $call_direction = $direction == 'inbound' ? 'outgoing' : 'incoming';
        if (isset($callerIdNumber)) {
            if (get_option('call_twilio_recording_active')) {
                $dial = $response->dial('', [
                    'callerId' => $callerIdNumber, 'record' => 'record-from-ringing-dual',
                    'recordingStatusCallback' => base_url() . 'lead_manager/Call_control/recordCall/' . $leadid . '?call_direction=' . $call_direction . '&is_lead=' . $is_lead . '&client_id=' . $client_id, 'recordingStatusCallbackMethod' => 'GET', 'recordingTrack' => 'both'
                ]);
...
public function handleNotRecordedIncomingCall($lead_id = null)
{
    $durationData = '';
    $call = '';
    $is_lead = $this->input->get('isLead');
    $call_direction = $this->input->get('Direction');
...
public function handleNotRecordedOutgoingCall($lead_id = null)
{
    ...
    if ($lead_id) {
        ...
        if ($is_lead) {
            ...
            $data['lead_id'] = $this->input->get('clientId');
            $data['staff_id'] = 0;
        }
        $response = $this->lead_manager_model->lead_manger_activity_log($data);
    }
...
public function get_lead_id_by_number($number)
{
    if ($number) {
        $CI = &get_instance();
        $CI->db->select('id');
        $CI->db->where(['phonenumber' => $number, 'clientId' => 0]);
...
{
    log_activity('recordCall : '.json_encode($_GET));
